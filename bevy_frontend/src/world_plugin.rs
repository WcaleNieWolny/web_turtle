                        ...
                        Some(world) => {
                            for block in &world.blocks {
                                 commands.spawn((PbrBundle {
                                    ...
                                    material: materials.add(Color::rgb_u8(block.r, block.g, block.b).into()),
                                    transform: Transform::from_xyz(0.5 + block.x as f32, block.y as f32 + 1.0, 0.5 + block.z as f32),
                                    ..default()
                                }, WorldBlock));
                            }
                        },
            ...
            Some(new_turtle) => {
                ...
                spawn_local(async move {
                    ...
                    if resp.status() != 200 {
                        ...
                        tx.try_send(None).expect("Cannot notify bevy world system (Err)");
                        return;
                    }

                    let json = JsFuture::from(resp.json().expect("Cannot get json")).await.expect("Cannot get future from JS");
                    let result: TurtleWorld = serde_wasm_bindgen::from_value(json).expect("Json serde error");
    ...
    for change in &mut world_change_events {
        ...
        match &change.action {
            shared::WorldChangeAction::New(new_block) => {
                 commands.spawn((PbrBundle {
                    mesh: meshes.add(shape::Cube { size: 1.0 }.into()),
                    material: materials.add(Color::rgb_u8(new_block.r, new_block.g, new_block.b).into()),
            ...
            shared::WorldChangeAction::Update(update) => {
                let block = world_blocks.iter()
                    .find(|(loc, _, _)| loc.translation.x == change.x as f32 + 0.5 && loc.translation.y == change.y as f32 + 1.0 && loc.translation.z == change.z as f32 + 0.5);
